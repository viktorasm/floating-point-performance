...
inline double normalize2dArrayOfVectors(const unsigned int numIterations, const unsigned int numVectors, const unsigned int vectorSize) {
	...
	iterationBuffers.resize(numIterations);
	...
	for (std::vector<VectorElementType **>::iterator iterationBufferPtr = iterationBuffers.begin(); iterationBufferPtr != iterationBuffers.end(); iterationBufferPtr++) {
		...
		for (VectorElementType **vertexWeightsPtr = *iterationBufferPtr, **end= vertexWeightsPtr+numVectors; vertexWeightsPtr != end; vertexWeightsPtr++) {
...
inline double normalizeArrayOfVectors(const unsigned int numIterations, const unsigned int numVectors, const unsigned int vectorSize) {
	...
	for (std::vector<VectorElementType *>::iterator iterationBufferPtr = iterationBuffers.begin(); iterationBufferPtr != iterationBuffers.end(); iterationBufferPtr++) {
		...
		// iterate through all influences
		for (VectorElementType *vertexWeights = *iterationBufferPtr, *end=vertexWeights+numVectors*vectorSize; vertexWeights != end; vertexWeights += vectorSize) {
			VectorElementType sum = 0;
			for (unsigned int i = 0; i < vectorSize; i++) {
				sum += vertexWeights[i];
			}
...
inline void compareImplementations(const char * const algorithmName, NormalizeFunc floatFunc, NormalizeFunc doubleFunc, const unsigned int numIterations, const unsigned int numVectors, const unsigned int vectorSize) {
	...
	double floatResult = normalizeArrayOfVectors<float>(numIterations, numVectors, vectorSize);
	double doubleResult = normalizeArrayOfVectors<double>(numIterations, numVectors, vectorSize);
...
int main() {
	...
	for (unsigned int verts = 1000, iterations = 10000; verts <= 100000; verts *= 10, iterations /= 10) {
		for (unsigned int influences = 1; influences <= 64; influences = influences<8?influences+1:influences+2) {
